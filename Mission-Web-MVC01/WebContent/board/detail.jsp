			...
			<table border="1" style="width: 80%">
				...
				<tr>
					...
					<td><c:forEach items="${fileList }" var="file">
							<a href="/Mission-Web/upload/${file.fileSaveName }"> <c:out
									value="${file.fileOriName }" />
							</a>											
						(${file.fileSize } bytes)						
						<button
								onclick="downloadImg('/Mission-Web/upload/${file.fileSaveName}')">
								download</button>
							<br>
				...
				<tr>
					<c:if test="${ not empty member }">
						<c:if test="${member.id eq board.writer }">
							...
							<td style="border: 0px solid white">
								<form onclick="return confirmDelete()" method="post"
									action="delete.jsp">
									<input type="hidden" name="no" value="${board.no}"> <input
					...
					<td style="border: 0px solid #D4E0EE;">
						<button
							onclick="location.href='<%=request.getContextPath() %>/board/list.do'">목록</button>
					</td>
				</tr>
			</table>
		</div>
